...
CYB3RHQ_CONFIG_MOUNT=/cyb3rhq-config-mount
...
function runOpensearchDashboards {
    ...
    /usr/share/cyb3rhq-dashboard/bin/opensearch-dashboards -c $OPENSEARCH_DASHBOARDS_HOME/config/opensearch_dashboards.yml \
        --cpu.cgroup.path.override=/ \
        --cpuacct.cgroup.path.override=/
}

mount_files() {
  if [ -e $CYB3RHQ_CONFIG_MOUNT/* ]
  then
    print "Identified Cyb3rhq cdashboard onfiguration files to mount..."
    exec_cmd_stdout "cp --verbose -r $CYB3RHQ_CONFIG_MOUNT/* $INSTALL_DIR"
  else
    print "No Cyb3rhq dashboard configuration files to mount..."
  fi
}
...
# Create and configure Cyb3rhq dashboard keystore
...
##############################################################################
# Start Cyb3rhq dashboard
##############################################################################

/cyb3rhq_app_config.sh $CYB3RHQ_UI_REVISION
