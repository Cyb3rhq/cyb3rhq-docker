...
export USER=${NAME}
export GROUP=${NAME}
export VERSION=${CYB3RHQ_VERSION}-${CYB3RHQ_TAG_REVISION}
...
export INDEXER_FILE=cyb3rhq-indexer-base.tar.xz
export BASE_FILE=cyb3rhq-indexer-base-${VERSION}-linux-x64.tar.xz
export REPO_DIR=/unattended_installer

## Variables
CERT_TOOL=cyb3rhq-certs-tool.sh
PASSWORD_TOOL=cyb3rhq-passwords-tool.sh
...
sed '/-Xms/d' -i /etc/cyb3rhq-indexer/jvm.options
sed '/-Xmx/d' -i /etc/cyb3rhq-indexer/jvm.options
sed -i 's/-Djava.security.policy=file:\/\/\/etc\/cyb3rhq-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/-Djava.security.policy=file:\/\/\/usr\/share\/cyb3rhq-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/g' /etc/cyb3rhq-indexer/jvm.options
